...
func newInitCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			cfg, err = runInitWizard(reader, out, errOut, cwd, cfg)
			if err != nil {
				return err
			}
			...
			fmt.Fprintln(out, "managed docs: context.md")
			return nil
		},
	}
}

func runInitWizard(reader *bufio.Reader, out io.Writer, errOut io.Writer, projectRoot string, cfg config.Config) (config.Config, error) {
	...
	if strings.TrimSpace(os.Getenv("RECALL_SUMMARIZER_CMD")) == "" && strings.TrimSpace(cfg.SummarizerCmd) == "" {
		fmt.Fprintln(errOut, "note: no summarizer command configured; auto-summarization is disabled")
	}
